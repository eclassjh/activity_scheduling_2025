...
"""
PFSP Makespan Minimization using OR-Tools CP-SAT

Author: Joohyun Lee
Date  : 2025-05-28

This script solves the classic Permutation Flow Shop Scheduling Problem (PFSP).
Given a set of jobs and machines, where each job must be processed in the same
order of machines, the goal is to find the job permutation that minimizes the
total completion time (makespan).

Key Components:
--------------
1. PFSP Class
   - Encapsulates logic to compute completion time for a given job permutation.

2. PermutationEvaluationCallback (CP-SAT Callback)
   - Called at each feasible solution during search.
   - Evaluates the makespan of each permutation.
   - Tracks the best solution found.

3. CP Model Construction
   - Uses integer variables to define job permutations.
   - Adds AllDifferent constraint to ensure valid orderings.

4. Main Function
   - Defines job processing data
   - Sets up and runs the CP-SAT solver
   - Prints best permutation and corresponding makespan
...
Usage:
------
Simply run this script to see all possible permutations evaluated,
and the best makespan found.
"""
