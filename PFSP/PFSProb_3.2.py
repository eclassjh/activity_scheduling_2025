...
# 제약조건 정의부 ;
# 뭔가 정의가 잘못 된 듯 함 ( 계산 결과가 이상했음 )
        # precedence 제약 (같은 job에서 기계 순서)
        for i in range(self.num_jobs):
            for j in range(1, self.num_machines):
                self.model.Add(self.start[i, j] >= self.end[i, j - 1])

        # 같은 기계에서 job 간 겹치지 않도록 제약
        for j in range(self.num_machines):
            for i1 in range(self.num_jobs):
                for i2 in range(i1 + 1, self.num_jobs):
                    b = self.model.NewBoolVar(f'before_{i1}_{i2}_m{j}')
                    self.model.Add(self.start[i1, j] >= self.end[i2, j]).OnlyEnforceIf(b)
                    self.model.Add(self.start[i2, j] >= self.end[i1, j]).OnlyEnforceIf(b.Not())
                    self.model.AddBoolOr([b, b.Not()])  # 한 방향은 반드시 참
