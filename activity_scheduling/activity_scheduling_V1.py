...
def print_workload_result(result_dict, cumulative_workload):
    ...
    # 결과 정렬 (시작 시간 순)
    sorted_acts = sorted(result_dict.items(), key=lambda x: x[1]['start'])
    
    total_end = 0
    for key, val in sorted_acts:
        start = val['start']
        end = val['end']
        duration = end - start
        total_end = max(total_end, end)
        print(f"Activity {key}: Start = {start}, End = {end}, Duration = {duration}")
...
def add_precedence_constraints(model, activity_dict, activity_var_dict): 
    ...
    for key, value in activity_dict.items():
        if value.predecessors is not None :
