...
def generate_new_solution(x, precedence_dict, activity_dict, max_tries=50) :
    n = len(x)
...
#         earliest = max(max(preds_start), activity_dict[act].earliest_start_date)
#         start_times[act] = earliest
...
def calculate_energy(x, activity_dict, precedence_dict):
    ...
    for action in x:
        ...
        # 선후행제약 1. 후행작업이 선행작업보다 늦게 시작해야 한다는 조건 
...
def run_SA(T, T_min, Niteration, log_interval=100) : #T=10, T_min=1e-3, Niteration=1000
    ...
    while iteration < Niteration or T > T_min : # 반복해 조건 정의
        x_new = generate_new_solution(x, precedence_dict, activity_dict) # x_new 정의
